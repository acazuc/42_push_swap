...
/*   Updated: 2016/02/10 16:53:18 by acazuc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		set_direction(t_env *env, t_sort *s)
{
	...
	{
		if (env->stack_b[i] == env->sorted[env->sorted_size - 1 - s->k])
		{
...
void			sort(t_env *env)
{
	...
	{
		if (env->stack_b[env->stack_b_size - 1] == env->sorted[env->sorted_size - 1 - s.k])
			place(env, &s);
		else
			move(env, &s);
	}
